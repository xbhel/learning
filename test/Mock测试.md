...
- stub依赖类的部分方法实现，而这些方法在你测试类和接口的时候会被用到，也就是说 stub 类在测试中会被实例化。stub 类会回应任何外部测试的调用。stub 类有时候还会记录调用的一些信息
...
- answers
  ...
  ```
  //没有stub时，指定返回格式
  Mockito.RETURNS_SMART_NULLS<==> Answers.RETURNS_SMART_NULLS
  ```

- Deep mock
- stub 类似于录制,而后面的调用叫做回放 ，简单说就是去 决定mock对象的行为
- `Mockito.rest(T mock) 销毁stub行为`